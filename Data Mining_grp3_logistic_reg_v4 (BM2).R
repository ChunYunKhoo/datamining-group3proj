...
for(i in 1:5) {
  ...
  #remove default, poutcome, duration, campaign in train set after cross-validation
  train_data <- subset(train_data, select= c(-default, -poutcome, -duration, - campaign))
  
  # Scale only the specified numeric columns
  numeric_cols_test <- c(1, 6, 10, 12, 13, 14, 15)   #no change to test data 
  numeric_cols_train <- c(1, 5, 9, 11, 12)   # change the columns
  ...
  # Calculate AUC
  auc <- auc(roc(test_data$y, log_test_pred))
...
for(i in 1:5) {
  ...
  #generating confusion matrix
  table_log1 <- table(Predicted = predicted_log_labels1, Reference = test_data1[,11])
  table_log1
...
test_pred_70 = get_logistic_pred(log_model, data = test_data, res = "y", 
...
test_tab_70 = table(predicted = test_pred_90, actual = test_data$y)

test_con_mat_10 = confusionMatrix(test_tab_10, positive = "yes")
test_con_mat_50 = confusionMatrix(test_tab_50, positive = "yes")
test_con_mat_70 = confusionMatrix(test_tab_90, positive = "yes")
...
rownames(metrics) = c("c = 0.10", "c = 0.50", "c = 0.70")
metrics
